...
class ReservaTest extends TestCase
{
    ...
    public function test_rejeita_sobreposicao_de_secretaria(): void
    {
        ...
        $this->criarReserva($dono, $secretaria, $periodo, 'pendente', $data);

        $response = $this->actingAs($outro)->post(route('reservas.store'), [
            'data' => $data,
            'periodo_id' => $periodo->id,
            'secretaria_id' => $secretaria->id,
        ]);
        ...
        $this->assertSame(1, Reserva::where('secretaria_id', $secretaria->id)->count());
    }
    ...
    public function test_rejeita_duplicacao_do_mesmo_utilizador(): void
    {
        ...
        $this->criarReserva($user, $secretaria1, $periodo, 'pendente', $data);
        ...
        $this->assertSame(1, Reserva::where('user_id', $user->id)->count());
    }
    ...
    public function test_cancelar_reserva(): void
    {
        ...
        $secretaria = $this->criarSecretaria();
        $periodo = $this->criarPeriodo();
        $reserva = $this->criarReserva($user, $secretaria, $periodo);
        $this->criarEstadoReserva('cancelada');

        $response = $this->actingAs($user)->delete(route('reservas.destroy', $reserva->id));
    ...
    public function test_impede_cancelar_reserva_de_outro_utilizador(): void
    {
        ...
        $secretaria = $this->criarSecretaria();
        $periodo = $this->criarPeriodo();
        $reserva = $this->criarReserva($dono, $secretaria, $periodo);

        $response = $this->actingAs($outro)->delete(route('reservas.destroy', $reserva->id));

        $response->assertNotFound();
        $this->assertNull($reserva->fresh()->cancelada_at);
    }
    ...
    public function test_impede_editar_reserva_de_outro_utilizador(): void
    {
        ...
        $response = $this->actingAs($outro)->get(route('reservas.edit', $reserva->id));
    ...
    public function test_impede_alterar_reserva_cancelada(): void
    {
        ...
        $secretaria = $this->criarSecretaria();
        $periodo = $this->criarPeriodo();
        $reserva = $this->criarReserva($user, $secretaria, $periodo, 'cancelada', null, now()->toDateTimeString());

        $response = $this->actingAs($user)->get(route('reservas.edit', $reserva->id));
    ...
    public function test_consultar_disponibilidade(): void
    {
        ...
        $this->criarReserva($user, $secretariaOcupada, $periodo, 'pendente', $data);

        $response = $this->actingAs($user)->getJson(
            route('reservas.availability', ['data' => $data, 'periodo_id' => $periodo->id])
        );

        $response->assertOk();
        $codigos = collect($response->json())->pluck('codigo');

        $this->assertTrue($codigos->contains($secretariaLivre->codigo));
        $this->assertFalse($codigos->contains($secretariaOcupada->codigo));
    }
    ...
    public function test_historico_e_paginado(): void
    {
        ...
        $response = $this->actingAs($user)->get(route('reservas.history'));
