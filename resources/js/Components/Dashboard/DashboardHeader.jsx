...
export default function DashboardHeader({ onOpenNav = () => { } }) {
    const { auth, reservaHojeUtilizador, proximasReservas, stats, notificacoesReais, notificacoesBD: notificacoesBDProp } = usePage().props;
    ...
    // 1. Notificações do Laravel (Database Notifications)
    const rawBDNotifications = notificacoesBDProp || auth?.unreadNotifications || auth?.user?.unread_notifications || [];
    const notificacoesBD = rawBDNotifications.map((notif) => ({
        id: notif.id,
        realId: notif.id,
        icon: CreditCard,
        titulo: notif.data?.titulo ?? 'Notificação',
        mensagem: notif.data?.mensagem ?? '',
        action_url: notif.data?.action_url ?? notif.data?.url ?? null,
        lida: !!notif.read_at,
        created_at: notif.created_at,
        isLaravelNotif: true,
    }));

    // 2. Notificações sintéticas por papel
    ...
    const notificacoesReaisMapeadas = notificacoesReaisEstado.map((notificacao) => ({
        ...
        action_url: notificacao.url ?? null,
        created_at: notificacao.created_at,
        isAppNotif: true,
    }));

    // 4. Junção e deduplicação
    const todasNotificacoes = [
        ...notificacoesBD,
        ...notificacoesReaisMapeadas,
        ...notificacoesSinteticas.map(n => ({ ...n, created_at: n.created_at || new Date().toISOString() })),
    ];

    const notificacoesUnicas = Array.from(
        new Map(
            todasNotificacoes.map((item) => {
                const tituloClean = (item.titulo || '').trim().toLowerCase();
                const mensagemClean = (item.mensagem || '').trim().toLowerCase();
                const fingerprint = `${item.realId || item.id}-${tituloClean}-${mensagemClean}`;
                return [fingerprint, item];
            })
        ).values()
    );

    const naoVistas = notificacoesUnicas.filter((notificacao) =>
    ...
    const handleDeleteNotificacao = (id) => {
        // Elimina no backend e filtra do estado local
        axios.delete(`/notificacoes/${id}`).then(() => {
            setNotificacoesReaisEstado((atual) => atual.filter((item) => item.id !== id && `real-${item.id}` !== id));
        }).catch(() => { });
    };
        ...
        <header className="mb-8 flex flex-col gap-6 xl:flex-row xl:items-center xl:justify-between">
            ...
            <div className="flex flex-wrap items-center gap-3">
                ...
                <div className="relative" ref={popupRef}>
                    ...
                    {notificationsOpen && (
                        <div className="absolute right-0 top-16 z-50 w-80 sm:w-96 rounded-2xl border border-slate-200 bg-white p-4 shadow-xl dark:border-slate-700 dark:bg-slate-900">
                            <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800">
                                ...
                                {naoVistas.length > 0 && (
                                    <span className="rounded-full bg-teal-500/10 px-2.5 py-1 text-xs font-semibold text-teal-600 dark:text-teal-400">
                                        {naoVistas.length} nova{naoVistas.length > 1 ? 's' : ''}
                                    </span>
                                )}
                            </div>
                            ...
                            {notificacoesUnicas.length > 0 ? (
                                <div className="mt-3 max-h-80 space-y-2.5 overflow-y-auto pr-1">
                                    {notificacoesUnicas.map((notificacao) => (
                                        <NotificationItem
                                            key={notificacao.id}
                                            notificacao={notificacao}
                                            onNotificationClick={() => setNotificationsOpen(false)}
                                            onDelete={(id) => handleDeleteNotificacao(id)}
                                        />
                                    ))}
                                </div>
                            ) : (
