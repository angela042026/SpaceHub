    ...
    return (
        ...
        >
            {selected ? (
                ...
                <>
                    <span
                        className={`flex h-[14.5px] w-[14.5px] items-center justify-center rounded-full text-[7px] font-bold text-white shadow-[0_1px_4px_rgba(15,23,42,0.35)] transition-all duration-[180ms] ease-out group-hover:scale-110 group-hover:shadow-[0_4px_10px_rgba(15,23,42,0.4)] group-focus-visible:scale-110 group-focus-visible:shadow-[0_4px_10px_rgba(15,23,42,0.4)] ${estado.bar}`}
                    >
                        {secretaria.numero}
                    </span>

                    <span
                        className={`pointer-events-none absolute bottom-full left-1/2 z-50 mb-2 flex -translate-x-1/2 items-center gap-1.5 whitespace-nowrap rounded-lg border border-slate-200/90 bg-white/95 px-2.5 py-1.5 text-slate-800 shadow-lg backdrop-blur transition-opacity duration-200 ${
                            hoverEnabled
