const ESTADOS = {
    livre: { cor: 'bg-status-livre', label: 'Livre' },
    reservada: { cor: 'bg-status-reservada', label: 'Parcialmente ocupada' },
    ocupada: { cor: 'bg-status-ocupada', label: 'Ocupada' },
    indisponivel: { cor: 'bg-status-indisponivel', label: 'Sem secretárias' },
};
...
export default function OfficeMap({ pisos, selectedFloor, setSelectedFloor }) {
    ...
    const setoresComPosicao =
        pisoAtual.setores?.filter((s) => s.planta_x !== null && s.planta_y !== null) ?? [];
        ...
        <div className="dashboard-card p-6">
            ...
            <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-white dark:border-slate-800">
                ...
                {setoresComPosicao.map((setor) => {
                    const estado = ESTADOS[setor.status] ?? ESTADOS.indisponivel;
            ...
            <div className="mt-4 grid grid-cols-1 gap-x-6 gap-y-2 text-sm sm:grid-cols-2">
                {pisoAtual.setores?.map((setor) => {
                    ...
                    return (
                        <div key={setor.id} className="flex items-center gap-2 text-slate-600 dark:text-slate-300">
                            <span
            ...
            <p className="mt-4 text-xs text-slate-400 dark:text-slate-500">
                Total de secretárias neste piso: {pisoAtual.totalSecretarias ?? 0}
            </p>
        </div>
    );
}
