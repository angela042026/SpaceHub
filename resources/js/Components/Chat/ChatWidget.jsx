    ...
    return (
        ...
        >
            ...
            {mounted && (
                <div
                    ref={popUpRef}
                    className={`fixed bottom-3 right-3 flex w-[min(320px,calc(100vw-24px))] max-h-[calc(100svh-24px)] flex-col overflow-hidden rounded-2xl bg-white shadow-[0_20px_60px_rgba(15,23,42,0.24)] transition-all duration-200 ease-out sm:absolute sm:bottom-[68px] sm:right-0 sm:w-[390px] sm:rounded-[20px] ${
                        isOpen
                            ? "translate-y-0 scale-100 opacity-100"
                            : "pointer-events-none translate-y-2 scale-95 opacity-0"
                    } ${
                        modoAtivo === "chat"
                            ? "h-[min(500px,calc(100svh-24px))] sm:h-[500px] sm:max-h-[75vh]"
                            : ""
                    }`}
                >
                    {modoAtivo === "menu" ? (
                        <>
                            {/* Cabeçalho */}
                            <div className="relative overflow-hidden bg-gradient-to-r from-[#03172B] to-[#14B8A6] px-4 py-4 sm:px-5 sm:py-5">
                                <button
                                    type="button"
                                    onClick={fechar}
                                    ...
                                    >
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div>
                                                <span className="block text-sm font-semibold text-[#071A33]">
                                                    {t(
                                                        "chat.assistenteVirtual",
                                                    )}
                                                </span>
                                                <span className="block text-xs text-slate-500">
                                                    {t(
                                                        "chat.respostasImediatas",
                                                    )}
                                                </span>
                                            </div>
                                        </div>
