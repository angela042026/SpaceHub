...
export default function Create({ edificios }) {
    ...
    const submit = (event) => {
        ...
        post(route('admin.pisos.store'), { forceFormData: true });
    };

    return (
        <DashboardLayout>
            <Head title={t('pisos.create.titulo')} />
                ...
                <div className="flex items-center gap-3 border-b border-slate-100 px-6 py-5 dark:border-slate-800">
                    ...
                    <div>
                        <h1 className="text-xl font-bold text-slate-900 dark:text-white">
                            {t('pisos.create.titulo')}
                        </h1>

                        <p className="text-sm text-slate-500 dark:text-slate-400">
                            {t('pisos.create.subtitulo')}
                        </p>
                    </div>
                </div>
                ...
                <form onSubmit={submit} className="p-6" noValidate>
                    <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
                        <div className="sm:col-span-2">
                            <label htmlFor="edificio_id" className={labelClass}>{t('campos.edificio')}</label>
                            <select
                                id="edificio_id"
                            ...
                            >
                                <option value="" disabled>{t('pisos.create.selecioneEdificio')}</option>
                        ...
                        <div>
                            <label htmlFor="numero" className={labelClass}>{t('campos.numero')}</label>
                        ...
                        <div>
                            <label htmlFor="planta" className={labelClass}>{t('campos.plantaImagem')}</label>
                            <label
                                htmlFor="planta"
                            ...
                            >
                                ...
                                {data.planta ? data.planta.name : t('campos.escolherFicheiro')}
                            </label>
                        ...
                        {preview && (
                            <div className="sm:col-span-2">
                                <img src={preview} alt={t('campos.previsualizacaoDaPlanta')} className="max-h-48 rounded-xl border border-slate-200 object-contain dark:border-slate-700" />
                            </div>
                        )}
                    ...
                    <div className="mt-8 flex items-center gap-3">
                        ...
                        >
                            {processing ? t('pisos.create.aCriar') : t('pisos.create.criar')}
                        </button>
